...
type Form
    ...
    | Sprite Number Number String Int Int
    | Words Color String
    | Group (List Shape)
...
renderAtlases : List Atlas -> Svg msg
renderAtlases ats =
    let
        symbols atlas =
            let
                aw =
                    (atlas.width - atlas.tileBorder)
                        // (atlas.tileWidth + atlas.tileBorder)

                ah =
                    (atlas.height - atlas.tileBorder)
                        // (atlas.tileHeight + atlas.tileBorder)
            in
            List.range 0 (ah - 1)
                |> List.concatMap
                    (\ay ->
